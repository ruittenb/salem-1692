 ...
 * into     [ 3, 6, -1 ]
 */
let arrayFilterSome = (elements: array<option<'a>>): array<'a> => {
 ...
 */
let option2AndThen = (maybeValue: option<'a>, fn: ('a => option<'b>)): option<'b> => {
 ...
 */
let option2Map = (maybeValue: option<'a>, fn: ('a => 'b)): option<'b> => {
 ...
 */
let safeExec = (functionThatMayThrow: () => 'a): option<'a> => {
    try {
