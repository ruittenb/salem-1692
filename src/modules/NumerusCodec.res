...
let numerusToJs = (n: numerus): string =>
  switch n {
  | One => "One"
  | More => "More"
  }

let numerusFromJs = (n: string): Js.Option.t<numerus> =>
  switch n {
  | "One" => Some(One)
  | "More" => Some(More)
  | _ => None
...
let decoder: Decco.decoder<numerus> = (json: Js.Json.t): Belt.Result.t<
  numerus,
  Decco.decodeError,
> => {
  switch json->Decco.stringFromJson {
  | Belt.Result.Ok(v) =>
    switch v->numerusFromJs {
