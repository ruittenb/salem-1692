...
let table = Js.Dict.fromArray([
    ...
    ("It is possible to join a game running on another telephone.", `Es ist möglich an einem Spiel teilzunehmen, das auf einem anderen Telefon läuft.`),
    (`Take the other telephone and look in the app under Settings → Game ID. Then enter the Game ID here.`, `Nehmen Sie das andere Telefon und schauen Sie in der App unter Einstellungen → ID des Spieles nach. Dann geben Sie hier die ID des Spieles ein.`),
